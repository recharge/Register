<script type="text/javascript">
	function postUpload(files) {
		for (var key in files) {
			var obj = files[key];
			$.post("http://registration.kidzart.com/api/bizcenterupload.php?url=" + obj.url + "&type=" + obj.data.type + "&name=" + obj.data.filename);
			$('#uploadButton').toggle();
		}
	}
</script>
<div class="row">
	...
	<?php
		...
		if ($_GET['q'] == "") {
			$ps = $pdo->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM bizcenter_files WHERE name LIKE ? OR tags LIKE ? ORDER BY name LIMIT $limit,$resultsPerPage");
    ...
    <div class="span10">
        <h2>Business Center <small>Showing <?php echo $limit+1 ?>-<?php echo min($limit+$resultsPerPage, $rows) ?> of <?php echo $rows ?></small></h2>
        ...
        <form action="/admin/bizcenter" method="get" style="margin:0;">
        ...
        <form class="form-search pull-right" style="margin-top: -31px;" action="/admin/bizcenter/" method="get">
...
<?php
	$ps = $pdo->prepare("SELECT * FROM bizcenter_files ORDER BY id");
	//$ps->execute();
	//$files = $ps->fetchAll();
?>
...
<?php foreach ($files as $file) { ?>
	<div class="modal hide fade" id="<?php echo $file['id'] ?>">
		<form action="/admin/bizcenter" method="post" style="margin:0;">
           	<input type="hidden" name="action" value="doEditBizCenterFile">
			...
			<div class="modal-footer">
				<a href="/admin/bizcenter?action=doDeleteBizCenterFile&id=<?php echo $file['id'] ?>" class="btn btn-danger pull-left">Delete File</a>
